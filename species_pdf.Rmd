...
# 
# db.init <- db.sp %>% select(c(sp_id:sp_nombres_comunes_portugues,
#                        group_sp_eval_subpob, sp_cat_nac_conserv_2019, 
#                        sp_bibliografia_citada:sp_kml_archivo))
# 
# db.sp.it <- db.sp %>% select(-c(sp_id:sp_nombres_comunes_portugues,
#                        group_sp_eval_subpob, sp_cat_nac_conserv_2019, 
#                        sp_bibliografia_citada:sp_kml_archivo)) %>% 
#   select(where(is.character))
# 
# db.sp.it <- map_df(db.sp.it, \(x) make_italics(x, c(Names ,paste0(LETTERS, '\\.'))))
#   # apply_italics_to_cols(db.sp.it, colnames(db.sp.it), Names)
# 
# db.sp <- bind_cols(
#   db.sp.it,  db.init,
#   db.sp %>%  select(-c(sp_id:sp_nombres_comunes_portugues,
#                        group_sp_eval_subpob, sp_cat_nac_conserv_2019, 
#                        sp_bibliografia_citada:sp_kml_archivo)) %>%
#     select(!where(is.character))
# )
...
tbsPob <- CMA_subpop_cons(db.sp, c(Names, paste0(LETTERS, '\\.')))
