...
```{r image, fig.pos='H',  fig.align='center', out.width='90%',fig.cap= paste0("Fotos por Roberto Cinti, Proyecto Pantano")}
photo <- list.files('photos/', '.jpg$', full.names = T, recursive = T)

filePhoto <- list.files('photos/', '.jpg$', full.names = F, recursive = T) %>% str_to_lower() %>% str_detect(gsub(' ', '-', str_to_lower(db.sp$title))) %>% which()
