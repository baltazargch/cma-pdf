...
Names <- Names[ Names != 'sp' ]

Names <- Names %>% atomic_names %>% make_pad

db.init <- db.sp %>% select(c(sp_id:sp_nombres_comunes_portugues, sp_bibliografia_citada, 
                              sp_bibliografia_de_referencia))

db.sp.it <- db.sp %>% 
  select(-c(sp_id:sp_nombres_comunes_portugues, sp_bibliografia_citada, 
                              sp_bibliografia_de_referencia)) %>% 
  select(where(is.character))

db.sp.it <- map_df(db.sp.it, \(x) make_italics(x, Names))
  # apply_italics_to_cols(db.sp.it, colnames(db.sp.it), Names)

db.sp <- bind_cols(
  db.sp.it,  db.init,
  db.sp %>%  select(-c(sp_id:sp_nombres_comunes_portugues, sp_bibliografia_citada, 
                              sp_bibliografia_de_referencia)) %>%  select(!where(is.character))
)

db.sp <- db.sp[, colnames(db)] %>% as_tibble()

write_csv(db.sp, 'tempdb.csv')
db.sp <- read_csv('tempdb.csv')
...
CMA_print_in_line(db.sp$sp_distribucion_historica_coment, 
...
CMA_print_in_line(db.sp$sp_abundancia_comentarios %>% 
                    str_replace_all('\n', '\n\n'), 
                  'Comentarios sobre la abundancia, densidad o probabilidad de ocupación de la especie', 'n\n')
